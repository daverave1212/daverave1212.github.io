	...
	function normalizeArrayTree(tree){
		if(tree == null) console.log("TREE STARTED NULL WTF MAN")
		let ret = []
		for(let i = 0; i<tree.length; i++){
			_normalizeArrayTree(tree[i], ret)
		}
		return ret
	}
	
	function _normalizeArrayTree(tree, arr){
		//console.log("Reading length of:")
		//console.log(JSON.stringify(tree))
		if(tree.length == null || typeof tree == "string"){
			arr.push(tree)
		} else {
			for(let i = 0; i<tree.length; i++){
				_normalizeArrayTree(tree[i], arr)
			}
		}
	}
