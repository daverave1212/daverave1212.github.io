	...
	function TextBox(X, Y, MaxWidth, Font, Canvas, LineHeight){
		...
		this.y = Y;
		this.lineSpacing = 20;
		if(LineHeight) this.lineSpacing = LineHeight;
		this.font = Font;
		this.text = "";
		
		
		const nlSeparator = "<br>"
		this.setText = function(t){
			...
			while(currentWord < wordList.length - 1){
				...
				while(getCanvasTextWidth(this.lines[currentLine] + " " + wordList[currentWord], this.canvas) < this.w && wordList[currentWord] != nlSeparator && currentWord < wordList.length){
					...
					currentWord++;}
				currentLine++;
				if(wordList[currentWord] == nlSeparator){
					currentWord++;}}
