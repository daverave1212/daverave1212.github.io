	...
	function getKeys(obj){
		ret = []
		forEachElementInObject(obj, function(o, k){
			ret.push(k)
		})
		return ret
	}
	...
	function functionNameToWords(fn){
		console.log(fn)
		let ret = ""
		let start = 0
		for(let i = 0; i<fn.length; i++){
			...
			if(isUpperCase(letter)){
				ret += fn.substring(start, i).toLowerCase() + " "
				start = i
			}
		}
		ret += fn.substring(start, fn.length).toLowerCase()
		return ret
	}
