	...
	gen.tavernDescription = function(townSize){
		...
		var nFloors				= randomInt(1, floor(nGuestRooms/randomInt(3, 5)));
		...
		if(!hasBrothel && !hasInsideToilet && !hasGarden && !hasVipRooms && !hasKitchen){
			...
			if(!hasVipRooms && !hasKitchen){
				desc += hasKitchen + " Yet, curiously, it does not seem to have a kitchen of it's own, nor does it have any choice regarding rooms, since all seem to be the same.";}
			else{
				if(!hasVipRooms && hasKitchen){
					desc += " The rooms seem bland, being all similar and almost not decorated at all";}
				else if(hasVipRooms && !hasKitchen){
					desc += " Curiously, it does not seem to have a kitchen to prepare food.";
				}
			}
		}
		desc += " With the " + nGuestRooms + " rooms split among " + nFloors + " floors,";
	...
	namegen.orcNameFemale 		= function(){
		var auxName = namegen.orcNameMale() + "a";
		return auxName;
	}
	...
	namegen.goodOrganization = function(){
		var q, w;
		q = randomOf(
			"Vigilants",
			"Exemplars",
	...
	namegen.thievesGuildName = function(){
		var q;
		var w;
	...
	namegen.banditGuildName = function(){
		var q = randomOf(
			"Blood",
		);
