	...
	function ThievesGuild(){
		this.name = namegen.thievesGuildName();
		this.leader = new NPC();}
	
	function Cult(){
		this.name = namegen.cultName();
		this.leader = new NPC();
	}
	
	function GoodGuild(){
		this.name = namegen.goodOrganization();
		this.leader = new NPC();
	}
	...
	function TownTrouble(evilOrganization){
		this.object = ""; /* vampire? bandits? necromancer? amulet? */
		this.description = "";
		this.problem = randomOf(
				"monster",
				"cult",
				"murder",
				"food",
		...
		switch(this.problem){
			case "monster":
				this.object = randomOf(
				"vampire",
				"strange undead crature",
				"demon",
				"evil necromancer",
				"sprite");
				this.description = "A(n) " + this.object;
				this.description += " is rumored to be roaming this town and causing trouble. Villagers are scared.";
				break;
			case "cult":
				/* TO DO LATER */
		}
	...
	function Town(type){
		...
		this.generateDescription = function(){
			...
			this.description += "<br><br>" + this.npc1.description;
			//this.description += "<br><br>" + this.npc2.description;
		...
		for(i = 1; i<=TOWNSNUMBER; i++){
			...
			while(!isPlaceable && nTries < 30){
				...
				if(newTown.collidesWithTowns()){
					...
					var doesItCollideWithWater = percentChance(ISLANDCHANCE); // Gives it 10% chance to be an island
